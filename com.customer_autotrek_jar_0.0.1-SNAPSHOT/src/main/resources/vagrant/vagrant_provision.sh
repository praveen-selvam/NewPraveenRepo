...
configure_mysql_server() {
    ...
    echo ${TEMP_ROOT_PASSWD}

    mysqladmin -u root -p"${TEMP_ROOT_PASSWD}" password "root_password"
    
    mysql -u root -proot_password -e "CREATE DATABASE atlocaldb2 DEFAULT CHARACTER SET = utf8 COLLATE utf8_general_ci"
        
       
    # Note this is not a production setup.
    mysql -u root -proot_password -e "GRANT ALL ON atlocaldb2.* TO 'atlocaldbuser2'@'%' IDENTIFIED BY 'atpassword'"
